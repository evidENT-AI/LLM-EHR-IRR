...
"""
Clinical Letter Analysis Module using Meta's LLAMA3.1 via Google Vertex AI

This module provides a LLAMA3.1-based implementation for analyzing clinical letters
via Google's Vertex AI API, with capabilities for preprocessing, annotation, and 
result management. It includes functionality for in-context learning and 
comprehensive logging of the analysis process.

The main class, LlamaInstance, handles:
    - Clinical letter preprocessing and loading
    - LLAMA3.1 interaction via Vertex AI API and prompt management
...
Example:
    ...
    with LlamaInstance(project_id=PROJECT_ID, location=LOCATION) as instance:
        letter = instance.load_letter("path/to/letter.json")
        results = instance.full_letter_annotation(letter)
    ```

Dependencies:
    - google.auth: For Google authentication
    - openai: For OpenAI-compatible API interface
...
Module requires snowstorm and elastic search servers to be running locally for SNOMED processing:
...
Author: Liam Barrett (adapted for LLAMA3.1)
Version: 1.0.0
"""
...
# Google and OpenAI imports for Vertex AI access to LLAMA3.1
import openai
...
class LlamaInstance:
    """Instance for LLAMA3.1 clinical letter analysis with preprocessing capability."""
    def __init__(self, 
                 project_id=None, 
                 location="us-central1", 
        ...
        # Determine if using the smaller model
        self.is_small_model = "8b" in self.model_name.lower()
        # Configure max output tokens based on model size - use config values
        from config import MODEL_PARAMS
        self.max_tokens = MODEL_PARAMS.get(model, {}).get('max_tokens', 8192)
    ...
    def initialize_state(self, letter_id=None):
        ...
        # Configure authentication for Vertex AI API
        credentials, _ = default()
        auth_request = transport.requests.Request()
        credentials.refresh(auth_request)
        
        # Configure OpenAI client to point to Vertex AI LLAMA3.1 endpoint
        self.client = openai.OpenAI(
            base_url=f"https://{self.location}-aiplatform.googleapis.com/v1/projects/{self.project_id}/locations/{self.location}/endpoints/openapi/chat/completions?",
        ...
        self.apply_llama_guard = True  # Default to using Llama Guard safety filter
        
        self.messages = []  # Keep separate track of messages for our logging
    ...
    def setup_logging(self):
        ...
        self.logger = logging.getLogger(f"LLAMA3_{self.letter_id}")
    ...
    def load_letter(self, file_path: str | Path, save=True) -> str:
        ...
        Args:
            ...
            save: Whether to save the loaded letter
    ...
    def set_pre_prompt(self, pre_prompt):
        ...
        # Reset conversation history for LLAMA3.1
        self.messages = [{"role": "system", "content": pre_prompt}]
        # Send system message via chat completions API
        response = self.client.chat.completions.create(
            model=self.model_name,
            messages=[{"role": "system", "content": "You are a medical expert specialized in analyzing clinical letters. " + pre_prompt}],
            temperature=self.temperature,
            extra_body={
                "extra_body": {
                    "google": {
                        "model_safety_settings": {
                            "enabled": self.apply_llama_guard,
    ...
    def prompt(self, message, validate_json=False):
        ...
        try:
            ...
            # Convert our internal message format to the format expected by the OpenAI API
            api_messages = []
            
            # Include previous messages (if any) to maintain context
            for msg in self.messages:
                if msg["role"] in ["system", "user", "assistant"]:
                    api_messages.append({"role": msg["role"], "content": msg["content"]})
            
            # Send request to LLAMA3.1 via Vertex AI
            response = self.client.chat.completions.create(
                model=self.model_name,
            ...
            # Store the assistant's response in our message history
            self.messages.append({"role": "assistant", "content": assistant_message})
            self.logger.info("Received response: %s", assistant_message)
...
"""
    ...
    def process_snomed_codes(self):
        ...
        snomed_output_dir.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            messages_state = self.messages.copy()  # Copy the current messages
                ...
                if results_file.exists():
                    try:
                        ...
                        if success:
                            ...
                            # We need to create a new chat for each SNOMED code selection
                            # to avoid context contamination
                            
                            # Restore messages list for our tracking
                            self.messages = messages_state.copy()
    ...
    def full_letter_annotation(self, preprocessed_letter):
        ...
        if self.is_small_model:
            EXAMPLE_LETTER_PROMPT_short = EXAMPLE_LETTER_PROMPT+"""
Remember, you must follow these rules when generating JSON:
1. Validate your JSON structure before submitting
2. Never repeat the same label multiple times
3. Complete all nested structures properly
...
"""
        try:
            if self.is_small_model:
                _ = self.prompt(EXAMPLE_LETTER_PROMPT_short, validate_json=False)
            else:
                _ = self.prompt(
                    EXAMPLE_LETTER_PROMPT,
                    validate_json=False
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        self.save_final_state()
        if exc_type:
            self.logger.error(f"Exception during processing: {exc_type}: {exc_val}")
        self.logger.info("Exiting LLAMA3.1 labeller instance")
