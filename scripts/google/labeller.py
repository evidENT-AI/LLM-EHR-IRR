...
"""
Clinical Letter Analysis Module using Google Gemini

This module provides a Gemini-based implementation for analysing clinical letters,
...
The main class, GeminiInstance, handles:
    - Clinical letter preprocessing and loading
    - Gemini interaction and prompt management
...
Example:
    ...
    with GeminiInstance(api_key) as gemini:
        letter = gemini.load_letter("path/to/letter.json")
        results = gemini.full_letter_annotation(letter)
    ```
...
The module requires a valid Google API key and appropriate permissions
...
class GeminiInstance:
    """Instance of Google Gemini for clinical letter analysis with preprocessing capability."""
    def __init__(self, api_key, model="gemini-1.5-pro", temperature=0, seed=42,
    ...
    def setup_logging(self):
        ...
        self.logger = logging.getLogger(f"Gemini_{self.letter_id}")
    ...
    def save_raw_response(self, response_text):
        ...
        response_file = self.output_dir / f"/raw_responses/raw_response_{timestamp}.txt"
    ...
    def set_pre_prompt(self, pre_prompt):
        ...
        # For Gemini, we'll restart the chat with a system message
        self.chat = self.client.chats.create(model=self.model_name)
        self.messages = [{"role": "system", "content": pre_prompt}]
        # Send system message as a user message since Google's API doesn't have a dedicated system role
        response = self.chat.send_message("You are a medical expert specialized in analyzing clinical letters. " + pre_prompt)
    ...
    def prompt(self, message, validate_json=False):
        ...
        try:
            ...
            # Configure the chat when creating it
            if not hasattr(self, 'chat') or self.chat is None:
                chat = self.client.chats.create(
                    model=self.model_name,
                )

            # Send the message without additional parameters
    ...
    def process_snomed_codes(self):
        ...
        try:
            ...
            # For Gemini API, we need to record the current chat ID
            chat_history = [msg for msg in self.chat.get_history()]
                ...
                if results_file.exists():
                    try:
                        ...
                        if success:
                            ...
                            # Reset conversation to post-analysis state for each code selection
                            # For Gemini, we need to start a new chat for each SNOMED code selection
