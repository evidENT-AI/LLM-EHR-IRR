...
class LLMOutputParser:
    ...
    def extract_json_from_text(self, text: str) -> Optional[str]:
        ...
        try:
            ...
            # If no markdown blocks, find text between first { and last }
    ...
    def parse_llm_output(self, llm_output: str, allow_partial: bool = False) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            # Handle both dictionary and list formats for labels
            cleaned_labels = {}
            
            if isinstance(labels, list):
                # If labels is a list, convert it to a dictionary using "text" as key
                for item in labels:
                    if 'text' in item:
                        text = item.pop('text')
                        cleaned_labels[text] = self.validate_label_entry(text, item)
                    else:
                        self.logger.warning("List item missing 'text' field, skipping")
            else:
                # Process as dictionary (original behavior)
