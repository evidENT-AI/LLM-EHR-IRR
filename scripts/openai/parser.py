...
class LLMOutputParser:
    ...
    def extract_json_from_text(self, text: str) -> Optional[str]:
        """Extract JSON string from LLM output text, handling markdown code blocks and truncation."""
        try:
    ...
    def parse_llm_output(self, llm_output: str, allow_partial: bool = False) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            # Handle different label formats: dict vs list
            if isinstance(labels, list):
                # Convert list format to dict format
                self.logger.info("Converting list-format labels to dict format")
                converted_labels = {}
                for item in labels:
                    if isinstance(item, dict) and 'text' in item:
                        # Use 'text' field as the key and remove it from the entry
                        text_key = item.pop('text')
                        converted_labels[text_key] = item
                    else:
                        self.logger.warning("Skipping invalid label item: %s", item)
                labels = converted_labels
            elif not isinstance(labels, dict):
                raise ValueError(f"Labels must be dict or list, got {type(labels)}")
            ...
            for label, entry in labels.items():
                try:
                    ...
                    self.logger.error("Error processing label %s", str(e))
                    continue
            return {"labels": cleaned_labels}
        except json.JSONDecodeError as e:
