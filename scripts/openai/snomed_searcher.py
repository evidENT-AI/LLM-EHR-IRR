...
class SnomedSearcher:
    ...
    def _create_simplified_results(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Prioritizes FSN term, then PT term, then base term.
        ...
        if "items" in results:
            for item in results["items"]:
                ...
                # Try to get FSN term first
                term = None
                if "fsn" in concept:
                    term = concept["fsn"].get("term")

                # If no FSN, try PT
                if term is None and "pt" in concept:
                    term = concept["pt"].get("term")

                # If neither FSN nor PT, use base term
                if term is None:
                    term = item.get("term")

                # Add to simplified results if we have both term and ID
